  ...
  public class Factory {
    ...
    public void Build(Datum headerDatum, Datum footerDatum, Control control) {
...
#if !TMP_PRESENT
      if (this.table.fontMaterial == null) {
        Shader s = Shader.Find("UI/Default Font");
        if (s == null) s = Shader.Find("UI/Default");
        this.table.fontMaterial = new Material(s);
      }
#endif
...
#if !(UNITY_5_0 || UNITY_5_1) && !UNITY_WEBGL
      ...
      if(this.table.bodyRect == null) {
        ...
        rt = go.AddComponent<RectTransform>();
        img = go.AddComponent<Image>();
        // we WANT this raycast so our body can be scrolled when no rows
    ...
    public void MakeRows() {
      ...
      float count = ((this.table.bodyRect.rt.rect.height - this.table.minHeaderHeight - this.table.minFooterHeight) / this.table.minRowHeight) + 1;
      //Debug.LogFormat(this.table, "Fasctory.MakeRows - (({0} - {1} - {2}) / {3}) + 1: {4}", this.table.bodyRect.rt.rect.height, this.table.minHeaderHeight, this.table.minFooterHeight, this.table.minRowHeight, count);
      for(int i = 0; i < count || i < this.table.rows.Count; i++) {
        if(this.table.rows.Count <= i) {
          ...
          row.extraText.material = this.table.fontMaterial;
#endif
          row.extraText.raycastTarget = false;
    ...
    private void MakeScrollbar(bool hor) {
      ...
      rt.transform.SetParent(this.table.root, false);
      ...
      rt.anchorMax = new Vector2(1, 1);
      rt.pivot = new Vector2(0.5f, 0.5f);
      rt.transform.SetParent(sb.transform, false);
      ...
      rt2.transform.SetParent(rt, false);
      rt2.offsetMin = new Vector2(0, 0);
      rt2.offsetMax = new Vector2(0, 0);
